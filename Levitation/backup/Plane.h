...
public:
	...
	{
		constants::zVEC3Normalize(&normal);
		plane.a = normal.x;
		plane.b = normal.y;
		plane.c = normal.z;
	...
	float getLengthOfNormalVec() const
	{
		...
		// check if 
		if (constants::areEqual(length, 1))
		{
			return 1;
		}

		if (constants::areEqual(length, 0))
		{
			return 0;
		}
	...
	*/
	float getDirectedDistanceToPlane(const zVEC3 & point) const
	{
		...
		if (constants::areEqual(result, 0)) {
			result = 0;
		}
		return result;
	}
	...
	*/
	bool isLineIntersecting(const Line& line) const
	{
		...
		return !constants::areEqual(scalar, 0);
	}
	...
	float getLambdaIntersectionValueForLine(const Line& line) const
	{
		...
		if (!constants::areEqual(lambdaCount, 0))
		{
